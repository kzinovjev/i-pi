...
def matrix_exp(M, ntaylor=20, nsquare=10):
    ...
    for i in range(ntaylor):
        tc[i + 1] = old_div(tc[i], (i + 1))

    SM = old_div(np.copy(M), 2.0**nsquare)
...
def h2abc(h):
    ...
    gamma = math.acos(old_div(h[0, 1], b))
    beta = math.acos(old_div(h[0, 2], c))
    alpha = math.acos(old_div(np.dot(h[:, 1], h[:, 2]), (b * c)))
...
def genh2abc(h):
    ...
    gamma = math.acos(old_div(np.dot(h[0], h[1]), (a * b)))
    beta = math.acos(old_div(np.dot(h[0], h[2]), (a * c)))
    alpha = math.acos(old_div(np.dot(h[2], h[1]), (b * c)))
...
def h2abc_deg(h):
    ...
    return a, b, c, old_div(alpha * 180, math.pi), old_div(beta * 180, math.pi), old_div(gamma * 180, math.pi)
...
def abc2h(a, b, c, alpha, beta, gamma):
    ...
    h[1, 2] = old_div((b * c * math.cos(alpha) - h[0, 1] * h[0, 2]), h[1, 1])
...
def eigensystem_ut3x3(p):
    ...
    for i in range(3):
        eigp[i, i] = 1
    eigp[0, 1] = old_div(-p[0, 1], (p[0, 0] - p[1, 1]))
    eigp[1, 2] = old_div(-p[1, 2], (p[1, 1] - p[2, 2]))
    eigp[0, 2] = old_div(-(p[0, 1] * p[1, 2] - p[0, 2] * p[1, 1] + p[0, 2] * p[2, 2]), ((p[0, 0] - p[2, 2]) * (p[2, 2] - p[1, 1])))
...
def exp_ut3x3(h):
    ...
    if (abs(old_div((h[0, 0] - h[1, 1]), h[0, 0])) > MINSERIES):
        r01 = old_div((e00 - e11), (h[0, 0] - h[1, 1]))
    else:
        r01 = e00 * (1 + (h[0, 0] - h[1, 1]) * (0.5 + (h[0, 0] - h[1, 1]) / 6.0))
    if (abs(old_div((h[1, 1] - h[2, 2]), h[1, 1])) > MINSERIES):
        r12 = old_div((e11 - e22), (h[1, 1] - h[2, 2]))
    else:
        r12 = e11 * (1 + (h[1, 1] - h[2, 2]) * (0.5 + (h[1, 1] - h[2, 2]) / 6.0))
    if (abs(old_div((h[2, 2] - h[0, 0]), h[2, 2])) > MINSERIES):
        r02 = old_div((e22 - e00), (h[2, 2] - h[0, 0]))
    else:
    ...
    eh[0, 2] = h[0, 2] * r02
    if (abs(old_div((h[2, 2] - h[0, 0]), h[2, 2])) > MINSERIES):
        eh[0, 2] += old_div(h[0, 1] * h[0, 2] * (r01 - r12), (h[0, 0] - h[2, 2]))
    elif (abs(old_div((h[1, 1] - h[0, 0]), h[1, 1])) > MINSERIES):
        eh[0, 2] += old_div(h[0, 1] * h[0, 2] * (r12 - r02), (h[1, 1] - h[0, 0]))
    elif (abs(old_div((h[1, 1] - h[2, 2]), h[1, 1])) > MINSERIES):
        eh[0, 2] += old_div(h[0, 1] * h[0, 2] * (r02 - r01), (h[2, 2] - h[1, 1]))
    else:
